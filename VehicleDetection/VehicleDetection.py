...
class VehicleDetection:
    ...
    def process_frame(self, frame):
        ...
        results = self.model.track(frame, persist=True, tracker="bytetrack.yaml")  # Perform vehicle tracking in the frame
        if results is not None and results[0] is not None and results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            line_thickness=2
            track_thickness=2
            annotator = Annotator(annotated_frame, line_width=line_thickness, example=str(names))
    ...
    def process_video(self, video_path, result_callback):
        ...
        while cap.isOpened():
            ...
            if success:
                ...
                cv2.imshow("YOLOv8 Tracking", self._decode_image_base64(response['annotated_frame_base64']))
